...
On login submission, the created hash is created from a mix of the username plus the ":" character and the password (the four digit pin). which ends up looking like this:
...
# User sessions should follow the structure {username, time out span in seconds, TOTP label and issuer}.
